...
import Mathematics.Exponentiation;
...
public class Primes {
    ...
    public static final Map<Long, Long> getPrimeFactorization(long number) {
        ...
        long n = number;  // Preserve original number for final check
        int c;
        ...
        for (long i = 2; i * i <= n; i++) {
            ...
            if (c > 0) {
                map.put(i, map.getOrDefault(i, 0L) + c);  // Add the count of factor i
            }
        }

        // If there's any prime factor larger than sqrt(number), it's the remaining n
        if (n > 1) {
            map.put(n, map.getOrDefault(n, 0L) + 1);  // Add the remaining prime factor
        }
